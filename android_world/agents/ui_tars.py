...
Action: {"action_type": "status", "goal_status": "infeasible"}"""
  ...
  def get_actions_androidworld_uitars(self, node:MCTSNode, task_goal:str, app_name:str):
    """
    ...
    images = []
    history_n = 4
    history_screenshot = history_screenshot[-history_n:]

    max_image_nums_under_32k = int(32768*0.75/max_pixels*28*28)
    if len(history_screenshot) > max_image_nums_under_32k:
        num_of_images = min(5, len(history_screenshot))
        max_pixels = int(32768*0.75) // num_of_images

    for turn, image in enumerate(history_screenshot):
        if len(images) >= 5:
            break
        try:
...
"""
    ...
    image_num = 0
    if len(history_action) > 0:
        for history_idx, history_response in enumerate(history_action):
            # send at most history_n images to the model
            if history_idx + history_n > len(history_action):
    ...
    for i in range(3):
      try:
        #todo: call ui-tars
        action_output, is_safe, raw_response = self.llm.predict_mm_message(
            messages
        )
        print("ui tars输出结果:")
        print(action_output)
