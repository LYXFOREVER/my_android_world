...
class Qwen2VLForConditionalGeneration(Qwen2VLPreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config):
        ...
        self.reward_mlp = nn.Sequential(
            ...
            nn.GELU(),
            nn.Linear(config.hidden_size // 4, 1),  # 映射为标量
        )
    ...
    def forward(
        ...
        reward_logits = self.reward_mlp(embed_features)
        # print(reward_labels.shape)
        # print(reward_logits.shape)
        loss = nn.BCEWithLogitsLoss()(reward_logits, reward_labels.reshape(-1, 1).to(reward_logits.dtype))
    ...
    def inference(self,
        ...
        embed_features = hidden_states[torch.arange(len(embed_indices)), embed_indices - 1] # (batch_size, embed_dim)
        reward_logits = self.reward_mlp(embed_features)
        prob = torch.sigmoid(reward_logits)
        return prob
