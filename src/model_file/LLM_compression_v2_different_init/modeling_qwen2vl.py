...
def find_OAS_segments(tensor):
    tensor = tensor.flatten()  # Ensure the tensor is one-dimensional

    O_starts = (tensor == 1906).nonzero(as_tuple=True)[0] # 1906 represents Image
    O_ends = []
    A_ends = []

    for i in range(len(O_starts)):
        O_start = O_starts[i]
        # Find the next 198 after O_start
        O_end_candidates = (tensor[O_start:] == 198).nonzero(as_tuple=True)[0]
        if len(O_end_candidates) > 0:
            O_ends.append(O_start + O_end_candidates[0])

        if len(O_ends) > i:  # If an O_end was found
            A_start = O_ends[i] + 1
            # Find the next 16448 after A_start
            A_end_candidates = (tensor[A_start:] == 16448).nonzero(as_tuple=True)[0]
            if len(A_end_candidates) > 0:
                A_ends.append(A_start + A_end_candidates[0])

    segments = []
    for i in range(len(O_starts)):
        if i < len(O_ends) and i < len(A_ends):  # If there is a corresponding A and S
            O_start = O_starts[i]
            O_end = O_ends[i]
            A_start = O_end + 1
            A_end = A_ends[i]
            S_start = A_end + 1
            S_end = O_starts[i+1].item() if i+1 < len(O_starts) else len(tensor) - 1
            segments.append([O_start.item(), O_end.item() + 1, A_start.item(), A_end.item() + 1, S_start.item(), S_end])
        else:  # If there is no corresponding A and S
            segments.append([])
...
def Find_OAS(tensor):
    ...
    result = []
    for i in range(batch_size):
        result.append(find_OAS_segments(tensor[i]))
    return result
    ...
    def Drop_hiddenstates(self, hidden, OAS_index, input_ids, position_embeddings):
        ...
        for index_list in OAS_index:
            ...
            for index in index_list:
                if index != []:
                    sample_OAS_num += index[3] - index[0]
            ...
            for j, index in enumerate(cur_index):
                if index != []:
                    if j == 0:
                        ...
                        cur_sequence.append(hidden_state[cur_index[j-1][3]:cur_index[j][0]])
                        cur_position_0.append(position_embeddings[0][:,i:i+1,cur_index[j-1][3]:cur_index[j][0]])
                        cur_position_1.append(position_embeddings[1][:,i:i+1,cur_index[j-1][3]:cur_index[j][0]])
                    
                    if j == (len(cur_index) - 2):
                        cur_sequence.append(hidden_state[cur_index[j][3]:])
                        cur_position_0.append(position_embeddings[0][:,i:i+1,cur_index[j][3]:])
                        cur_position_1.append(position_embeddings[1][:,i:i+1,cur_index[j][3]:])
    ...
    def forward(
        ...
        for i, decoder_layer in enumerate(self.layers):
            ...
            if not self.training:
                if i == 3 and hidden_states.shape[1] != 1:
                    hidden_states, position_embeddings, padded_attention_mask = self.Drop_hiddenstates(hidden_states, OAS_index, input_ids_aux, position_embeddings)
...
class Qwen2VLForConditionalGeneration(Qwen2VLPreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config):
        ...
        self.summary_num = 4
        self.summary = nn.ParameterList([nn.Parameter(torch.randn(64, 1536)) for _ in range(self.summary_num)])
        self.model = Qwen2VLModel(config)
        self.vocab_size = config.vocab_size
        self.lm_head = nn.Linear(config.hidden_size, config.vocab_size, bias=False)
        self.padding_side = "left"  # set it to left by default, user can use setter to change padding_sides
        for i in range(self.summary_num):
            torch.nn.init.xavier_uniform_(self.summary[i])
        # Initialize weights and apply final processing
        self.post_init()
    ...
    def Replace_summary(self, input_embs, input_ids):
        ...
        # print(all_index)
        sum_index = torch.tensor(list(range(self.summary_num-1, -1, -1)))
        for i, index in enumerate(all_index):
            if len(index) != 0:
                if len(index) == 1:
                    his = index[0]
                    """add fake gradient here to ensure gradient graph with one batch is same"""                    
                    input_embs[0, 0:64] += self.summary[0] * 0 + self.summary[1] * 0 + self.summary[2] * 0 + self.summary[3] * 0
                    input_embs[i, his[0]: his[0] + 64] = self.summary[0]
                else:
                    sum_index_sample = sum_index[-len(index):].tolist()
                    for j, his in enumerate(index):
                        # input_embs[i] = torch.concat((input_embs[i, :his[0]],self.summary,input_embs[i, his[0] + 64:]),dim=0)
                        input_embs[i, his[0]: his[0] + 64] = self.summary[sum_index_sample[j]]
                    """add fake gradient here to ensure gradient graph with one batch is same"""                    
                    input_embs[0, 0:64] += self.summary[0] * 0 + self.summary[1] * 0 + self.summary[2] * 0 + self.summary[3] * 0
                ...
                for j, index in enumerate(cur_index):
                    if index != []:
                        if j == 0:
                            ...
                            cur_label.append(label[cur_index[j-1][3]:cur_index[j][0]])

                        if j == (len(cur_index) - 2):
                            cur_label.append(label[cur_index[j][3]:])
