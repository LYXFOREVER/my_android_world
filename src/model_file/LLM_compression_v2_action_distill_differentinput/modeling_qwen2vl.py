...
def find_OAS_segments(tensor):
    ...
    for i in range(len(O_starts)):
        if i < len(O_ends) and i < len(A_ends):  # If there is a corresponding A and S
            ...
            A_start = O_end + 1
            A_end = A_ends[i]
            segments.append([O_start.item(), O_end.item() + 1, A_start.item(), A_end.item() + 1])
...
class Qwen2VLForConditionalGeneration(Qwen2VLPreTrainedModel, GenerationMixin):
    ...
    def forward(
        ...
        if inputs_embeds is None:
            ...
            inputs_embeds_student = self.model.embed_tokens(input_ids)
            ...
            if pixel_values is not None:
                ...
                pixel_values_student = pixel_values_student.type(self.visual.get_dtype())
                image_embeds_student = self.visual(pixel_values_student, grid_thw=image_grid_thw)
                image_embeds_student = image_embeds_student.to(inputs_embeds.device, inputs_embeds.dtype)
                inputs_embeds_student = inputs_embeds_student.masked_scatter(image_mask, image_embeds_student)
        ...
        full_image_outputs = self.model.full_forward(
            input_ids=None,
            position_ids=position_ids,
            attention_mask=attention_mask,
            past_key_values=past_key_values,
        ...
        full_image_hidden_states = full_image_outputs[0]
        full_image_logits = self.lm_head(full_image_hidden_states)
        full_image_logits = full_image_logits.float()
        ...
        # teacher is full image logits
        KL_labels = labels.clone()
        logits_teacher = full_image_logits[KL_labels!=-100]
        logits_student = logits[padded_label!=-100]
        
        student = F.softmax(logits_student, dim=-1)
        teacher = F.softmax(logits_teacher, dim=-1)

        KL_loss = F.kl_div(student.log(), teacher, reduction="batchmean")

        loss = KL_loss * self.alpha


        # v1 means only drop3 loss
        # v2 means only full forward loss
        # v3 means drop3 loss + full forward loss
        if self.version == 1 or self.version == 3:
                ...
                loss_v1 = loss_fct(shift_logits, shift_labels)

        if self.version == 2 or self.version == 3:
                # Shift so that tokens < n predict n
                shift_logits = full_image_logits[..., :-1, :].contiguous()
                ...
                # Enable model parallelism
                shift_labels = shift_labels.to(shift_logits.device)
                loss_v2 = loss_fct(shift_logits, shift_labels)
