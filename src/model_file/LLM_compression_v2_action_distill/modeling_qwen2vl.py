...
class Qwen2VLForConditionalGeneration(Qwen2VLPreTrainedModel, GenerationMixin):
    ...
    def forward(
        ...
        if self.version == 2 or self.version == 3:
                # Shift so that tokens < n predict n
                second_shift_logits = full_image_logits[..., :-1, :].contiguous()
                second_shift_labels = labels.clone()[..., 1:].contiguous()
                # Flatten the tokens
                loss_fct = CrossEntropyLoss()
                second_shift_logits = second_shift_logits.view(-1, self.config.vocab_size)
                second_shift_labels = second_shift_labels.view(-1)
                # Enable model parallelism
                second_shift_labels = second_shift_labels.to(second_shift_logits.device)
                loss_v2 = loss_fct(second_shift_logits, second_shift_labels)
        ...
        if self.version == 1:
            ...
            loss += (self.b1 * loss_v1 + self.b2 * loss_v2)
