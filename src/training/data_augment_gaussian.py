...
from my_qwen_vl_utils_gaussian import process_vision_info_with_resize, process_vision_info_with_resize_ROI, process_vision_info_with_mask
...
def get_image_info(image_path, min_pixel, max_pixel, point=None, mask=False, mask_prob=0.5, window_min=0.1, window_max=0.5, 
                   mask_level = None, sigma = 10, mask_area_min = 0.5, mask_area_max = 0.7):
    ...
    if mask:
        ...
        image_input, _ = process_vision_info_with_mask(messages, **kwargs)
    else:
...
class SupervisedDataset(Dataset):
    ...
    def __init__(
        ...
        self.mask_level = data_args.mask_level
        self.sigma = data_args.sigma
        self.mask_area_min = data_args.mask_area_min
        self.mask_area_max = data_args.mask_area_max
        ...
        if "image" in sources:
            ...
            for idx, image_file in enumerate(image_files):
                if idx == len(image_files) - 1:
                    ...
                    images.append(get_image_info(image_file, self.min_pixel, self.max_pixel, point, self.augment, 
                                             self.mask_prob, self.window_min, self.window_max,
                                             self.mask_level, self.sigma, self.mask_area_min, self.mask_area_max))
                else:
                    point = None
