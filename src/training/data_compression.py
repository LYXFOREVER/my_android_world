...
from my_qwen_vl_utils_ import process_vision_info_with_resize
...
def filter_ids(prompt_input_ids, start_id=4913, end_id=16448):
    result = []
    keep = False  # 是否处于保留状态
    
    for i, id in enumerate(prompt_input_ids):
        if id == start_id:  # 遇到起始ID
            keep = True
        if id == end_id and keep:  # 遇到结束ID，关闭保留状态
            # if prompt_input_ids[i+1] == 151653:
            #     result.append(151653)
            #     result.append(198)
            keep = False
        if keep:
            result.append(id)  # 保留当前ID
        else:
            result.append(-100)  # 替换为-100
...
class SupervisedDataset(Dataset):
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        for idx, j in enumerate(range(0, len(sources), 2)):
            ...
            # use compressed token num rather than the original
            # 这里调整input_ids, labels和mask
            # 把<|vision_start|>和<|vision_end|>之间的pad token数目调整 151652 151653
            ...
            # filter_tensor = torch.tensor(filter_ids(prompt_input_ids[0]))
