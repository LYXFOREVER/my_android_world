...
from my_qwen_vl_utils_ import process_vision_info_with_resize, process_vision_info_with_resize_ROI, process_vision_info_with_mask

from .params import DataArguments
from .constants import *

def process_padding(input_ids):
    ...
    # Find the start index of the last segment
    for i in range(len(input_ids) - 1, -1, -1):
        if input_ids[i] == 151652:
    ...
    while i < len(input_ids):
        ...
        if id == 151652:
            ...
            if i > last_segment_start:
                # Add all 151655 for the last segment
                result.extend([151655] * count_151655)
            else:
                # Add up to 64 151655 for non-last segments
                result.extend([151655] * min(count_151655, 64))
            result.append(id)
        elif in_segment and id == 151655:
            count_151655 += 1
        else:
            result.append(id)
        i += 1
...
def get_image_info_ROI(image_path, point, mask=False, mask_prob=0.5):
    # Using this because of process_vision_info function
    # Need to fix this in the future    
    ...
    if mask:
        image_input, _ = process_vision_info_with_mask(messages, mask_prob)
    else:
        image_input, _ = process_vision_info_with_resize_ROI(messages)
...
class SupervisedDataset(Dataset):
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        for idx, j in enumerate(range(0, len(sources), 2)):
            ...
            prompt_input_ids = process_padding(prompt_input_ids)
            prompt_input_ids = torch.tensor(prompt_input_ids, dtype=response_input_ids.dtype).to(response_input_ids.device)
