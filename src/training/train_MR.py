...
from training.data_MR import make_supervised_data_module
...
def configure_vision_tower(model, training_args, compute_dtype, device):
    vision_tower = model.MR_encoder
    vision_tower.to(dtype=compute_dtype, device=device)

    vision_model_params = model.MR_encoder.vit_encoder.parameters()
    set_requires_grad(vision_model_params, not training_args.freeze_vision_tower)
    
    pix_model_params = model.MR_encoder.pix_encoder.parameters()
    set_requires_grad(pix_model_params, not training_args.freeze_vision_tower)

    # Handle merger specifically
    merger_params = model.MR_encoder.vit_encoder.merger.parameters()
...
def train():
    ...
    if not training_args.lora_enable:
        ...
        if not training_args.vision_lora:
            training_args.lora_namespan_exclude += ["MR_encoder"]
    ...
    if hasattr(model.base_model.model.MR_encoder, "align_stages_latent"):
        print("unfreeze align_stages_latent")
        for name, param in model.base_model.model.MR_encoder.align_stages_latent.named_parameters():
            param.requires_grad = True
        # for name, param in model.base_model.model.MR_encoder.align_stages.named_parameters():
        #     param.requires_grad = True
